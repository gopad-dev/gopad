...
func (s *SyntaxLayers) HighlightIter(ctx context.Context, source []byte, r *buffer.ByteRange) iter.Seq2[HighlightEvent, error] {
	...
	for _, layer := range s.layers.Map() {
		...
		queryCaptures := newQueryCapturesIter(cursor.Captures(layer.Config.Query, layer.Tree.RootNode(), source))
