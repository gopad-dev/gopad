...
var DefaultStyles = Styles{
	Style:   lipgloss.NewStyle().Padding(0, 1).Margin(0, 1).Reverse(true),
	Focused: lipgloss.NewStyle().Padding(0, 1).Margin(0, 1).Foreground(lipgloss.ANSIColor(13)).Reverse(true),
}
...
func (m Model) View() string {
	if m.focus {
		return zone.Mark(m.zoneID(), m.Styles.Focused.Render(m.Label))
	}

	return zone.Mark(m.zoneID(), m.Styles.Style.Render(m.Label))
}
